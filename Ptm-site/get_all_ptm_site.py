...
UniprotId_all_file_path = '../inter_intra_cross_talk_proteins(rep)_620.txt'
...
# supply_dataset_file_path = '../Ptm-site/Supply.xlsx'  # 用来补充PTM type的信息
...
#
#
# Inter_Cross_talk_path = '../Datasets/Sup_Cross-talk.xlsx'
#
...
# Sup_Inter_Cross_talk_Proteins_Pairs = []
# with open('../Datasets/Proteins/inter_cross_talk_proteins_pairs(sup).txt', 'r', encoding='utf-8') as f2:
#     lines = f2.readlines()[1:]
#     for line in lines:
#         Sup_Inter_Cross_talk_Proteins_Pairs.append(line.strip('\n').split('\t'))

Intra_Inter_Cross_talk_Proteins_Pairs = []
for uniprotId1 in uniprotIds:
    for uniprotId2 in uniprotIds:
        if uniprotId2 != uniprotId1:
            if (uniprotId1, uniprotId2) not in Intra_Inter_Cross_talk_Proteins_Pairs and (uniprotId2, uniprotId1) not in Intra_Inter_Cross_talk_Proteins_Pairs:
                Intra_Inter_Cross_talk_Proteins_Pairs.append((uniprotId1, uniprotId2))

Intra_Inter_Cross_talk_Proteins_Pairs_1 = []

for i in range(len(uniprotIds)):
    for j in range(i+1, len(uniprotIds)):
        uniprotId1 = uniprotIds[i]
        uniprotId2 = uniprotIds[j]
        Intra_Inter_Cross_talk_Proteins_Pairs_1.append((uniprotId1, uniprotId2))

print('Intra_Inter_Cross_talk_Proteins_Pairs', len(Intra_Inter_Cross_talk_Proteins_Pairs))
print('Intra_Inter_Cross_talk_Proteins_Pairs_1', len(Intra_Inter_Cross_talk_Proteins_Pairs_1))


Ptm_site_all_file_path = './Ptm_site_all.xlsx'
Ptm_site_Ltp_file_path = './PTM_LTP.xlsx'

# 下面统计下86个蛋白质的所有低通量PTM位点，不包含高通量
...
print('11:', len(uniprotId_sites_ltp_dic.keys()))
...
print(len(uniprotId_sites_ltp_dic.keys()))
uniprotId_site_all = [item[1] for item in uniprotId_sites_ltp_dic.keys()]
print(len(uniprotId_site_all))

for uniprotId in uniprotIds:
    if uniprotId not in uniprotId_site_all:
        print(uniprotId)
with open('./uniprotId_sites_Ltp_Intra_Inter.txt', 'w') as f2:
...
for uniprotId1, uniprotId2 in Intra_Inter_Cross_talk_Proteins_Pairs:
    try:
        uniprotId1_name, uniprotId1_ptms = get_protein_name_ptms(uniprotId1, uniprotId_sites_ltp_dic)
        uniprotId2_name, uniprotId2_ptms = get_protein_name_ptms(uniprotId2, uniprotId_sites_ltp_dic)
    except:
        continue
    # print(values)
    print(uniprotId1, uniprotId2)
    # print(uniprotId1_ptms, '\n', uniprotId2_ptms)
    # print(list(itertools.product([1, 2, 3], [5, 6, 7])))
    for item in list(itertools.product(uniprotId1_ptms, uniprotId2_ptms)):
        print(item)
        # with open('./neagtive_Ltp_Intra_Inter_14.txt', 'a') as f3:
        #     f3.write(uniprotId1_name[0] + '\t' + uniprotId1_name[1] + '\t' + item[0][0] + '\t' + item[0][1] + '\t'
        #              + uniprotId2_name[0] + '\t' + uniprotId2_name[1] + '\t' + item[1][0] + '\t' + item[1][
        #                  1] + '\n')
        with open('./neagtive_Ltp_Intra_Inter_14_NoGENE.txt', 'a') as f3:
            f3.write(uniprotId1_name[1] + '\t' + item[0][0] + '\t' + item[0][1] + '\t'
                     + uniprotId2_name[1] + '\t' + item[1][0] + '\t' + item[1][
                         1] + '\n')

# 下面统计下86个蛋白质的所有PTM位点，包含了低通量和高通量
